...
export function TeamCard({ name, role, image, socials }: { name: string, role: string, image: string, socials: { Icon: LucideIcon | TablerIcon, url: string }[] }) {
  return (
    <div className="flex flex-col items-center">
      <div className="relative w-64 h-64 rounded-full overflow-hidden bg-zinc-900/50 text-zinc-300 grid">
          <div
           className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-zinc-900/50 text-zinc-300"
          >
              <Camera size={64} />
          </div>
        <Image
